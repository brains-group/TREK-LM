...
RATING_FILE_NAME = dict({'movie-1m': 'ratings.dat'})
SEP = dict({'movie-1m': '::'}) #设置分隔符
THRESHOLD = dict({'movie-1m': 4})
...
def convert_rating(DATASET, item_index_old2new, entity_id2index):
    ...
    for line in open(file, encoding='latin-1').readlines()[1:]:
...
def convert_kg(DATASET, entity_id2index):
    ...
    files = []
    if DATASET == 'movie-1m':
        files.append(open('../data/' + DATASET + '/kg_part1.txt', encoding='latin-1'))
        files.append(open('../data/' + DATASET + '/kg_part2.txt', encoding='latin-1'))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='movie-1m', help='which dataset to preprocess')
