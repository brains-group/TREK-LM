...
class Corpus:
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        for iters in range(1):
            ...
            ranks.extend(ranks_head)
            ranks.extend(ranks_tail)
        ...
        print(
            "Hits@100 are {} +- {}".format(
                cumulative_hits_100, calcStandardError(cumulative_hits_100)
            )
        )
        print(
            "Hits@10 are {} +- {}".format(
                cumulative_hits_ten, calcStandardError(cumulative_hits_ten)
            )
        )
        print(
            "Hits@3 are {} +- {}".format(
                cumulative_hits_three, calcStandardError(cumulative_hits_three)
            )
        )
        print(
            "Hits@1 are {} +- {}".format(
                cumulative_hits_one, calcStandardError(cumulative_hits_one)
            )
        )
        print(
            "Mean rank {} +- {}".format(
                cumulative_mean_rank,
                np.std(average_mean_rank_head + average_mean_rank_tail),
            )
        )
        print(
            "Mean Reciprocal Rank {} +- {}".format(
                cumulative_mean_recip_rank,
                np.std(average_mean_recip_rank_head + average_mean_recip_rank_tail),
            )
        )

        precision = sum([rank <= 10 for rank in ranks]) / sum(
            [min(rank, 10) for rank in ranks]
        )
        print(f"Precision: {precision} +- {calcStandardError(precision)}")
        recall = sum([rank <= 10 for rank in ranks]) / len(ranks)
        print(f"Recall: {recall} +- {calcStandardError(recall)}")
        f1_score = (
            2 * (precision * recall) / (precision + recall)
            if (precision + recall) > 0
            else 0
        )
        print(f"F1-Score: {f1_score} +- {calcStandardError(f1_score)}")
