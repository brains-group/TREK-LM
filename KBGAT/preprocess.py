...
def init_embeddings(entity_file, relation_file):
    ...
    return np.array(entity_emb, dtype=np.float32), np.array(
        relation_emb, dtype=np.float32
    )
...
def load_data(
    filename, entity2id, relation2id, is_unweigted=False, directed=True, partition=None
):
...
def build_data(
    ...
    test_triples, test_adjacency_mat, unique_entities_test = load_data(
        os.path.join(path, "test.txt"), entity2id, relation2id, is_unweigted, directed
    )
